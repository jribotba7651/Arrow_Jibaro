...
final class GameEngineTests: XCTestCase {
    ...
    func testTapEscapesAndWins() {
        var state = makeState([TestBoards.straight(0, .right, head: (0, 1), length: 2)], size: 4)
    ...
    func testTapBlockedLosesLife() {
        ...
        let outcome = GameEngine.tap(at: Position(row: 0, col: 1), in: &state)
        XCTAssertEqual(outcome, .blocked(pieceID: 0, by: 1))
        XCTAssertEqual(state.lives, 2)
    }

    func testTapEmptyCellIgnored() {
        var state = makeState([TestBoards.straight(0, .right, head: (0, 1), length: 1)], size: 4)
